...
from .utils import scraper_spectra,airtex,scraper_usmotorworks,scraper_densoautoparts,scraper_carter
import csv
import boto3
import time
...
def webmotors(self, duration):
    ...
    url = "https://{0}.s3.amazonaws.com/{1}/{2}".format(bucketname, folder, filename)
    print(url)
    File.objects.create(name="Usmotor",url="sdsd")
    return url
...
def autoparts(self, duration):
    ...
    f = open("autoparts.csv", "r", encoding='utf-8')
    g =f.read()
    #
    a = File.objects.all().count()+1
